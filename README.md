...
Each PDF is extracted by a VLM in a single structured call against a swappable Pydantic schema. SKU-level records are served via Elasticsearch at a blended ~$0.00131 per processed PDF — comfortably under the $0.002 target. Scope grows from 34 → 600 categories and 10 → 600 attributes as a prompt change, not a pipeline change. Two architectures coexist: **VLM-first** (§1.1) runs today as a single-call extraction; **Hybrid** (§1.2) is staged for migration if VLM economics break, and unlocks parse-once + diff-extract savings precisely because its parse step is non-LLM.

---

# 1. End-to-End Architecture

## 1.1 VLM-First Architecture
...
### 1.1.5 Post-Processing & Validation
...
### 1.1.8 SQS — Burst Buffering and Fault Tolerance
...
## 1.2 Hybrid Architecture — Alternate When Economics Break
...
| SKU extraction | Gemini 2.5 Flash-Lite (Pydantic structured output) | 1..N SKU records read off the stored canonical Markdown + active schema; full or diff-mode per §1.2.1 |
...
### 1.2.1 Parse Once, Extract Many — Hybrid's Cost Lever
...
Gemini Embedding 2 is a multimodal embedding model that projects text and images into a shared vector space. It generates embeddings at $0.00012/image, stored as the `image_embedding` dense_vector. Supports reverse image search (upload product photo → find matching SKUs).
...
| Avg input tokens per extraction call (4–5 pages incl. 1–2 product images) | ~7,000 |
| Avg output tokens per extraction call (structured JSON, 1..N SKUs) | ~1,000 |
| Cycles per year | 26 |
...
|---|---|---|---|
| Extract — single call (PDF → SKU JSON) | $0.001100 | $0.000550 | $0.000567 |
...
| **Total** | **~$0.00207** | **~$0.00128** | **~$0.00131** |

**vs $0.002 target**

| **Scenario** | **Cost / PDF** | **vs $0.002 target** |
|---|---|---|
| All fast path (worst case) | ~$0.00207 | 4% over |
| All slow path (best case) | ~$0.00128 | 36% under |
| **Blended (97% slow + 3% fast)** | **~$0.00131** | **34% under target** |
...
| Hybrid path (fallback) | PaddleOCR + LayoutReader + LangChain Agent + `difflib` | Activated if Gemini is deprecated/repriced (§1.2); cheap deterministic parse makes the canonical Markdown worth persisting to S3, which in turn enables diff-extract — VLM called only on detected charts/tables |
...
| VLM hallucinates numeric fields (wattage, CCT, lumens) — wrong value indexed as `Present` | Medium | High | Range/outlier validation in post-processing; per-attribute F1 tracked separately for numeric fields; periodic offline eval on stratified sample; cross-field consistency checks (e.g. wattage × efficacy = lumens); citations to ground every value and make hallucinations detectable; manufacturer-prior validation to flag values outside expected distribution for a given brand |
...
| Gemini 2.5 Flash-Lite deprecated or repriced — blended cost target broken | Medium | High | Migrate to Hybrid architecture (§1.2) |
...
| Prompt injection via adversarial PDF content — extraction output manipulated | Low | Medium | Injection-shielding in all prompts; Pydantic-validated structured output; PDF text sanitized before re-feeding into prompts |
...
**Migration triggers** (weekly ops review — no page): See §1.2 for the full list.
